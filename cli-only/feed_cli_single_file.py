...
def _write_cfg(cfg_path: Path, bind_ip: str, paths: List[str], tls_key: Optional[str] = None, tls_cert: Optional[str] = None) -> Dict[str, str]:
    """Generate mediamtx.yml at cfg_path and return credential dict."""
    creds = _get_credentials()
    config = _create_config(bind_ip, paths, creds, tls_key, tls_cert)
...
def _print_urls(host: str, paths: List[str], creds: Dict[str, str], rtsps: bool = False) -> None:
    for i, path in enumerate(paths):
        ...
        if rtsps:
            ...
            typer.secho("📲 Encrypted RTSPS Publishing:", fg=typer.colors.CYAN, bold=True)
...
def _load_config_credentials(config_path: Path) -> Dict[str, str]:
    ...
    try:
        ...
        auth_users = config.get("authInternalUsers", [])
        if not auth_users:
            typer.secho("No auth users found in config", fg=typer.colors.RED)
            raise typer.Exit(1)

        # Find publisher and viewer users
        pub = next(u for u in auth_users if any(p.get("action") == "publish" for p in u.get("permissions", [])))
        view = next(u for u in auth_users if any(p.get("action") == "read" for p in u.get("permissions", [])))
        return {
...
def _load_config_paths(config_path: Path) -> List[str]:
    ...
    try:
        ...
        if not paths_config:
            typer.secho("No paths found in config", fg=typer.colors.RED)
            raise typer.Exit(1)
...
def run(
    ...
    if config:
        ...
        config_paths = _load_config_paths(cfg_path)
    ...
    with temp_context as tmpdir:
        if not config:
            ...
            creds = _write_cfg(cfg_path, bind, paths, tls_key=tls_key_path, tls_cert=tls_cert_path)
            config_paths = paths
        ...
        _print_urls(host_ip, config_paths, creds, rtsps=use_rtsps)
